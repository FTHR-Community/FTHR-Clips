...
| **Hotkeys** | Global hotkeys via the XDG GlobalShortcuts portal (KDE, GNOME 48+) or compositor binds (Linux) and RegisterHotKey (Windows); Windows supports controller chords |
...
- `grim` for screenshots; `nc`/netcat only for the compositor-bind hotkey
  fallback on desktops without a GlobalShortcuts portal backend
...
FTHR never reads input devices on Linux: root access and membership in the
`input` group are not required, and the app cannot see keys it has not asked
for.

On desktops with a GlobalShortcuts portal backend (KDE Plasma 6, GNOME 48+,
Hyprland with `xdg-desktop-portal-hyprland`) FTHR registers its actions with the
desktop, which owns the key grab and lists them in its own shortcut settings.
Once the desktop knows a shortcut it keeps that key; change it there
(**Settings → Hotkeys → Edit in desktop settings** opens the dialog). Because
the portal identifies apps by their `.desktop` file, the AppImage installs
`~/.local/share/applications/fthr-clips.desktop` on first run and starts inside
its own systemd scope.

Without a portal backend FTHR falls back to a private owner-only Unix socket
under the user runtime directory. Hyprland bindings are generated by the app.
Traditional `hyprland.conf` sessions use the generated `fthr-hotkeys.conf`
source; Lua-based Hyprland sessions use the managed FTHR block in the loaded Lua
keybind configuration. Only the FTHR-owned block is replaced, so unrelated user
configuration is preserved. Reload Hyprland after changing bindings if the
compositor does not pick them up automatically. Other compositors do not import
Hyprland binds: use the exact command shown in **Settings → Hotkeys** and
configure that compositor's global shortcut facility.
...
- **Hotkeys do nothing:** on KDE/GNOME check the desktop's shortcut settings
  for the "FTHR Clips" entries (the log shows `[Portal] GlobalShortcuts
  session` when the portal path is active); on Hyprland confirm the generated
  configuration is loaded and inspect `hyprctl binds`.
