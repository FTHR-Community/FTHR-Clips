...
If the AppImage cannot mount because FUSE is unavailable, try the portable
extraction fallback:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./build_output/FTHRClips-<version>-x86_64.AppImage
```

For a support report, run this without `sudo` and attach the output together
with the FTHR log:

```bash
bash tools/linux_appimage_report.sh ./build_output/FTHRClips-<version>-x86_64.AppImage > appimage-report.txt
```

The report includes the artifact checksum, architecture, session type, display
server indicators, FUSE availability, and feature helper paths while omitting
private configuration values. The Linux AppImage is currently x86_64-only.
